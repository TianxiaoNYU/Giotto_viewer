...
.then(function(text2){
	...
	tsne_layer.eachLayer(function (layer) {
		...
		layer.on('click', function (e) {
   	 	$("#search_box").val(tx.text());
			activate_cell_type(layer);
		});
		var tx = $("a[id='" + layer.options.id + "']");
		tx.click(function(e){
			$("#search_box").val(tx.text());
			setHighlight(layer);
...
/*
$("#tsne_cell_fill_cluster").click(function(e){


});

$("#tsne_cell_fill_expression").click(function(e){
	//var ret_l = segmentation_group.getLayersByIDs(cell_ids);
	...
	var ret_l = tsne_layer.getLayersByIDs(cell_ids);
	for(i=0; i<ret_l.length; i++){
		var cid = tsne_layer._layers[i].options.id;
		t_expr = Number(current_expr[cid]);
		if(t_expr>2.0){
			t_expr = 2.0;
		}else if(t_expr<0){
			t_expr = 0;
		...
		//viewMode = "expression";
		tsne_layer._layers[i].setStyle({fill: true, fillColor:rgbToHex(t_r,255-t_r,255-t_r), fillOpacity:1.0});
	}
});

$("#tsne_cell_fill_none").click(function(e){
...
/*
$("#option_expression").click(function(e){
	if(this.checked){
		viewMode = "expression";
...
.then(function(text2){
	...
	Object.keys(map_cell).forEach(function (cell_id){
		...
		//alert(latlngs);
		var c1, c2;
		var t_cen = getCentroid2(map_cell[cell_id]);
		c1 = t_cen[0];
		c2 = t_cen[1];
		...
		var rad = [];
		for(i=0; i<18; i++){
			rad.push(i * 20.0 * Math.PI / 180.0);
		}
		var latlngs2 = [];
		for(i=0; i<rad.length; i++){
			var x = Math.cos(rad[i]) * radius + c1;
			var y = Math.sin(rad[i]) * radius + c2;
			var latlng = map.unproject([x,y], map.getMaxZoom());
			latlngs2.push([latlng.lat, latlng.lng]);
		}
		//alert(c1 + " " + c2);
		//var marker = L.circle(map.unproject([c1, c2], map.getMaxZoom()), {
		//	radius: 30, color: "red", fillOpacity: 1.0, weight:1, 
		//	stroke: true, "id": cell_id,
		//});
		var polygon = L.polygon(latlngs2, {color:"red", weight:1, fill:false, "id":cell_id, "type": "polygon"});
		//zoom6layer.addLayer(polygon);
		segmentations.push(polygon);
		//segmentations.push(marker);
	});
